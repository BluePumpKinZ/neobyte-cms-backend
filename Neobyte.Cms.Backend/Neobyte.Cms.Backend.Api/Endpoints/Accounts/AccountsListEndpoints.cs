...
public class AccountsListEndpoints : IApiEndpoints {
	...
	public void RegisterApis (RouteGroupBuilder routes) {
		...
		routes.MapPost("create/with-password",
				async ([FromServices] AccountManager manager, [FromBody] AccountsWithPasswordCreateRequestModel request
				) => {
					var result = await manager.CreateAccountWithPasswordAsync(request);
					if (!result.Success)
						return Results.BadRequest(result.Errors);

					return Results.Ok(new {Message = "Created", Id = result.AccountId!.Value.Value});
				}).Authorize(UserPolicy.OwnerPrivilege)
			.ValidateBody<AccountsWithPasswordCreateRequestModel>();
